...
select cursist
from inschrijvingen
where cursus = 'XML' and cursist in (select cursist
                                     from inschrijvingen
                                     where cursus = 'JAV');
...
from uitvoeringen
where begindatum in (select begindatum
                     from uitvoeringen
...
select voorl, naam
from medewerkers
where chef in (select u.docent
               from uitvoeringen u
                        join cursussen c
                             on c.code = u.cursus
               where c.type = 'ALG')
  and mnr not in (select cursist
                  from inschrijvingen)
  and functie = 'TRAINER';
